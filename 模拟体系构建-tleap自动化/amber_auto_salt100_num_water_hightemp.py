...
import re
import argparse
from sltcap_module_number_water import run_sltcap
...
tleap_commands = [
    ...
    "solvatebox Mpro TIP4PEWBOX 12.0",
...
for index, cmd in enumerate(tleap_commands): 
    ...
    if cmd == "charge Mpro":
        print(f"\n>>> Sending command: {cmd}")
    if cmd == "solvatebox Mpro TIP4PEWBOX 12.0":
        print(f"\n>>> Sending command: {cmd}")

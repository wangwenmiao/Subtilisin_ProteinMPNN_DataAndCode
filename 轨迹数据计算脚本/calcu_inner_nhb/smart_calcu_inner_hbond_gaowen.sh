...
for idx in "${!dirs[@]}"; do
    ...
    for i in {1..3}; do
        ...
        if [[ -d "$md_dir" && "$md_dir" == *apo* ]]; then
            ...
            mpirun -n 24 cpptraj.MPI -i /home/yons/wwm/4519_subtilisin_120ns_ace_aapfa_nme_HID/calcu_inner_nhb/inner_hbond_apo_gaowen.cpp
            cd - > /dev/null
        fi
        ...
        if [[ -d "$md_dir" && "$md_dir" == *holo* ]]; then
            ...
            mpirun -n 24 cpptraj.MPI -i /home/yons/wwm/4519_subtilisin_120ns_ace_aapfa_nme_HID/calcu_inner_nhb/inner_hbond_holo.cpp
            cd - > /dev/null
        fi
    done
